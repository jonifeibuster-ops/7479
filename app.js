    ...
    <div class="product-card__top">
      <div>
        <div class="product-card__name">
          <a href="product.html?id=${product.id}" class="product-card__link">${product.name}</a>
        </div>
...
function initCartPage() {
  ...
  function renderCart() {
    ...
    cart.forEach((item) => {
      ...
      row.innerHTML = `
        <div>
          <div class="cart-item__name"><a href="product.html?id=${product.id}">${product.name}</a></div>
...
function initLoginPage() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    setTimeout(() => {
      window.location.href = currentRole === 'admin' ? 'admin.html' : 'index.html';
    }, 700);
  });
}
...
function initAdminPage() {
  ...
  if (!user || user.role !== 'admin') {
    panels.hidden = true;
    msg.innerHTML =
      'Доступ ограничен. Войдите как администратор на странице <a href="login.html">Вход</a> (пароль: admin123).';
    return;
  }
...
function initWishlistPage() {
  ...
  function render() {
    ...
    if (!wishlist.length) {
      ...
      empty.className = 'cart-empty';
      empty.innerHTML = 'Избранное пусто. <a href="catalog.html">Перейти в каталог</a>';
      container.appendChild(empty);
      return;
    }
